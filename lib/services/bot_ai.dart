...
class BotAI {
  ...
  static Future<void> useBotSpecialPower(GameState gameState,
    ...
    if (val == '7') {
      ...
      if (target != null && target.hand.isNotEmpty) {
        ...
        debugPrint(
            "🔄 Bot échange sa carte #$myBadCardIdx avec la carte #$targetIdx de ${target.name}");

        // ✅ MODIFIÉ : Notification avec pause du timer
        if (target.isHuman && _context != null) {
          // Mettre le timer en pause
          final gameProvider =
              Provider.of<GameProvider>(_context!, listen: false);
      ...
      List<Player> possibleTargets = List.from(gameState.players);
      possibleTargets.shuffle(Random());
      Player? target = possibleTargets.first;
  ...
  static bool _shouldCallDutch(
      GameState gs, Player bot, BotDifficulty difficulty) {
    int estimatedScore = bot.getEstimatedScore();

    debugPrint(
        "🎯 Score estimé du bot: $estimatedScore (seuil Dutch: ${difficulty.dutchThreshold})");
  ...
  static Future<void> _decideCardAction(
    ...
    debugPrint(
        "🎴 [_decideCardAction] Carte piochée: ${drawn.value} (${drawn.points} pts)");
    ...
    for (int i = 0; i < bot.mentalMap.length; i++) {
      if (bot.mentalMap[i] != null) {
        ...
        if (mentalCardValue > drawnVal) {
          replaceIdx = i;
          debugPrint(
              "🔄 [_decideCardAction] Carte mentale plus haute trouvée: index $i (${mentalCardValue} pts)");
          break;
        }
      }
    }
    ...
    if (replaceIdx == -1) {
      ...
      debugPrint(
          "❓ [_decideCardAction] Carte inconnue choisie: index $replaceIdx");
    }

    // Décision : remplacer ou défausser
    if (replaceIdx != -1 && drawnVal < 8) {
      debugPrint(
          "✅ [_decideCardAction] DÉCISION: REMPLACER (index $replaceIdx)");
  ...
  static Player? _findTargetPlayer(GameState gameState, Player me) {
    try {
      return gameState.players.firstWhere((p) => p.isHuman && p.id != me.id,
  ...
  static BotDifficulty _getDifficultyFromPersonality(
