...
Dernière mise à jour : 2026-07-01 (Codex)
...
> ⚠️ **NE PAS RELANCER D'ENTRAÎNEMENT SUR L'ANCIEN CHECKPOINT.** Le runner avait des BLOCKERs de fidélité (corrigés) et l'ancien reward v2 ne pénalisait pas les faux matchs (**H2**, corrigé par `fix(rl): scalarize v2 rewards`). Audit/patch reward : `docs/ai/RL_REWARD_V2_AUDIT.md`. Le reward R2D2 v2 est maintenant scalarisé côté Python via `rl/reward_v2.py` avec match outcomes directs et Dutch suicide fortement pénalisé : `p0 successful_match=+0.5` immédiat, `p0 false_match=-0.7` immédiat, `WIN_REWARD=+1.0`, perte normale `0.0`, Dutch raté p0 `FAILED_DUTCH_PENALTY_REWARD=-1.0`, `destab` reste un petit potentiel payé seulement si `p0` gagne. Les events adverses restent publics mais ne récompensent/pénalisent pas p0.
...
- `7a45f92 fix(rl): gate positive v2 rewards on wins` : tentative win-gated. Le clean run from scratch a montré une dégénérescence inverse : Dutch trop tôt (`failed Dutch` 29/30).
- Patch follow-up Codex : reward match directe et Dutch suicide corrigés : `successful_match=+0.5`, `false_match=-0.7`, failed Dutch p0 `-1.0`.
...
Le cœur R2D2 v2, le runner corrigé et le reward v2 direct-match/failed-Dutch sont en place. **Avant tout nouveau training**, revalider les tests de garde-fou sur l'environnement cible, puis lancer une collecte neuve sur le runner corrigé avec cette reward. Séquence recommandée :
...
Ne pas relancer de training si la reward diverge de la règle “gagner la manche ; matches corrects payés directement ; faux match et Dutch raté coûtent toujours ; Dutch raté ne doit jamais être une échappatoire rentable”.
...
- **Reward v2 direct-match + failed Dutch fort** (patch Codex 2026-07-01, après `7a45f92`) : objectif optimisé = gagner la manche sans échappatoire Dutch suicide. `principal` Dart reste stocké comme diagnostic, mais n'est plus additionné directement au total optimisé. Formule effective : `p0 successful_match=+0.5` immédiat, `p0 false_match=-0.7` immédiat, terminal gagnant `WIN_REWARD=+1.0` (+ `win_bonus` Dart si victoire), perte normale `0.0`, Dutch raté p0 `FAILED_DUTCH_PENALTY_REWARD=-1.0`. Les matches adverses restent visibles dans `recent_events` (`actor=p1`, etc.) mais donnent `0.0` à p0. `destab` reste un petit potentiel win-gated (`max(0, clip(destab, 0, 2))/256`). La finalisation d'épisode vit dans `rollout_v2.finalize_episode_rewards_v2` et est utilisée par rollout/collecte/inférence ; les traces d'inférence sont écrites après finalisation pour ne pas reporter une reward terminale provisoire.
...
Contexte :
- Le premier clean run from scratch après reward win-gated
  (`/tmp/dutch_r2d2_v2_clean_reward_run_20260630_233140`) a supprimé le
  match-spam mais révélé une dégénérescence inverse : modèle greedy
  `average_steps=6.33`, `dutch_calls=29/30`, `successful Dutch=0`,
  `failed Dutch=29`, `wins=0`.
- Diagnostic : le bon match win-gated était trop subtil ; l'agent préférait
  perdre vite avec peu de pénalités. Dutch raté à `-0.25` restait trop faible
  pour empêcher cette stratégie d'évitement.

Changement :
- `rl/reward_v2.py` paie désormais `p0 successful_match` directement :
  `SUCCESSFUL_MATCH_REWARD=+0.5`.
- `p0 false_match` vaut `FALSE_MATCH_PENALTY_REWARD=-0.7`, donc un faux match
  coûte plus cher qu'un bon match ne rapporte.
- `FAILED_DUTCH_PENALTY_REWARD=-1.0` pour `p0` caller perdant, avec fallback si
  l'observation terminale est ambiguë mais que l'action sélectionnée est
  `call_dutch`.
- Les matches et Dutch adverses restent des événements publics (`actor`) mais ne
  donnent ni reward ni malus à p0.
- `destab` reste un petit potentiel win-gated ; `principal` reste diagnostic.

Validation attendue avant run :
- `test_reward_v2.py` couvre direct match `+0.5`, faux match `-0.7`, events
  adverses sans reward p0, Dutch raté `-1.0`, Dutch réussi positif, pas de
  double-count, métriques evaluate, trace reward components et anti-fuite.
- Ne pas lancer de training/collecte tant que cette suite et les tests v2
  ciblés ne sont pas verts.
