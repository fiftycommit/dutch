...
import { ArrowLeft, Zap, Users, Bot, Sparkles } from "lucide-react";
...
export function QuickGameScreen({ onBack, sbmmEnabled = false }: QuickGameScreenProps) {
  ...
  const handleStart = () => {
    console.log("Starting quick game:", { botLevel, playerCount, sbmmEnabled });
    ...
    <div className="size-full flex flex-col relative overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="p-3 landscape:p-2 rounded-2xl bg-emerald-600 backdrop-blur-xl">
            <Zap className="size-6 landscape:size-5 text-white" />
          </div>
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 md:px-6 lg:px-8 landscape:px-3 py-6 md:py-8 landscape:py-2 landscape:overflow-y-auto">
        <div className="w-full max-w-2xl space-y-8 md:space-y-12 landscape:space-y-3">
          ...
          >
            ...
            <div className="flex justify-center gap-2 md:gap-3 landscape:gap-2">
              {[2, 3, 4, 5, 6].map((count) => (
                <motion.button
                  key={count}
                  whileHover={{ scale: 1.05 }}
                  ...
                  className={`size-12 md:size-16 landscape:size-10 rounded-xl md:rounded-2xl landscape:rounded-lg font-bold text-lg md:text-xl landscape:text-base transition-all ${
                    playerCount === count
                      ? "bg-emerald-600 text-white shadow-lg scale-105"
                      : "bg-white/60 backdrop-blur-xl text-emerald-800 hover:bg-white/80"
                  }`}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={handleStart}
            className="w-full max-w-xs mx-auto block py-4 md:py-5 landscape:py-3 px-8 rounded-2xl md:rounded-3xl landscape:rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-lg md:text-xl landscape:text-base shadow-2xl transition-colors"
          >
            COMMENCER
          </motion.button>
        </div>
